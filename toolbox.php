...
class Agendamentos
{
	...
	function changeAllEmail()
	{
		...
		if($date < $timenow)
		{
			$query = mysql_query("SELECT * FROM `accounts` WHERE (`email_date` != '0' and `email_date` < '$timenow')") or die(mysql_error());
			while ($fetch = mysql_fetch_array($query))
			{	
				$updatedAccounts++;
				mysql_query("UPDATE accounts SET email = '".$fetch['new_email']."', new_email = '', email_date = '0' WHERE id = '".$fetch['id']."'") or die(mysql_error());	
			}
